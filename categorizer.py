...
def filter_predictions_using_threshold(dataset: Dataset, threshold: float) -> Dataset:
    ...
    for image in dataset_filtered.images:
        image.predictions = [
...
def match_and_classify(dataset: Dataset, min_iou: float = 0.0) -> Dataset:
    ...
    for image in dataset_matched_and_classified.images:
        ...
        dataset_matched_and_classified.n_tp += image.n_tp
        dataset_matched_and_classified.n_fn += image.n_fn
        dataset_matched_and_classified.n_fp += image.n_fp
